...
import videocomposer.utils.logging as logging
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        print(f"get item {index} from dataset")
        ...
        if os.path.exists(video_key):
            try:
                ...
                print(f"ref_frame: {ref_frame.device, ref_frame.shape, ref_frame.dtype}")
                print(f"vit_image: {vit_image.device, vit_image.shape, vit_image.dtype}")
                print(f"video_data: {video_data.device, video_data.shape, video_data.dtype}")
                print(f"misc_data: {misc_data.device, misc_data.shape, misc_data.dtype}")
                print(f"misc_data: {mv_data.device, mv_data.shape, mv_data.dtype}")
    ...
    def _get_video_traindata(self, video_key, feature_framerate, total_frames, visual_mv):
        ...
        for _ in range(5):
            try:
                ...
                print(f"frame_types: {len(frame_types), frame_types}")
                print(f"frames: {len(frames), frames[0].shape, frames[0].dtype}")
                print(f"mvs: {len(mvs), mvs[0].shape, mvs[0].dtype}")
                print(f"mvs_visual: {len(mvs_visual), mvs_visual[0].shape, mvs_visual[0].dtype}")
                # os.remove(filename)
                break
            except Exception as e:
        ...
        print(f"start_indexs: {start_indexs}")
        ...
        print(f"have_frames: {have_frames}")
        if have_frames:
            ref_frame = frames[middle_indix]
            print(f"middle_indix: {middle_indix}")
            vit_image = self.vit_transforms(ref_frame)
            print(f"vit_image: {vit_image.shape}")
            misc_imgs_np = self.misc_transforms[:2](frames)
            print(f"misc_imgs_np: {len(misc_imgs_np)}")
            misc_imgs = self.misc_transforms[2:](misc_imgs_np)
            print(f"misc_imgs: {len(misc_imgs)}")
        ...
        print(f"video_data: {len(video_data)}")
        print(f"mv_data: {len(mv_data)}")
        print(f"misc_data: {len(misc_data)}")
        if have_frames:

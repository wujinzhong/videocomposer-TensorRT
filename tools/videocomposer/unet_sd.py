...
def sinusoidal_embedding(timesteps, dim, tensor_arange_160):
    # check input
    half = dim // 2
    assert dim==320 and half==160

    timesteps = timesteps.float()

    import nvtx
    rng = nvtx.start_range(message="p0", color="red")
    # compute sinusoidal embedding
    sinusoid = torch.outer(
        timesteps,
        #torch.pow(10000, -torch.arange(half).to(timesteps).div(half)))
        torch.pow(10000, -tensor_arange_160.to(timesteps).div(160)))
    nvtx.end_range(rng)
    rng = nvtx.start_range(message="p1", color="red")
    x = torch.cat([torch.cos(sinusoid), torch.sin(sinusoid)], dim=1)
    nvtx.end_range(rng)
    rng = nvtx.start_range(message="p2", color="red")
    if dim % 2 != 0:
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        print(f"_ATTN_PRECISION: {_ATTN_PRECISION}")
...
class UNetSD_temporal(nn.Module):
    ...
    def __init__(self,
        ...
        self.tensor_arange_160 = torch.arange(160, dtype=torch.float32, device=torch.device('cuda:0')) #move here from "forward()": torch.arange(half).to(timesteps)
        self.context_gpu = torch.zeros(1, 0, 1024).to(torch.device('cuda:0')) #move from 'forward()': context = x.new_zeros(batch, 0, self.context_dim)
    ...
    def forward(self, 
        ...
        import nvtx
        rng = nvtx.start_range(message="p0", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p1", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p2", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p3", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p4", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p5", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p6", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p7", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p8", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p9", color="blue")
        ...
        nvtx.end_range(rng)
        rng10 = nvtx.start_range(message="p10", color="blue")

        rng = nvtx.start_range(message="p10_0", color="blue")
        ...
        # embeddings
        rng = nvtx.start_range(message="p10_1", color="blue")
        if self.use_fps_condition and fps is not None:
            e = self.time_embed(sinusoidal_embedding(t, self.dim, self.tensor_arange_160)) + self.fps_embedding(sinusoidal_embedding(fps, self.dim, self.tensor_arange_160))
        else:
            e = self.time_embed(sinusoidal_embedding(t, self.dim, self.tensor_arange_160)) 
        nvtx.end_range(rng)

        nvtx.end_range(rng10)
        rng = nvtx.start_range(message="p11", color="blue")

        assert batch==1 and self.context_dim==1024
        #context = x.new_zeros(batch, 0, self.context_dim)
        context = self.context_gpu
        if y is not None:
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p12", color="blue")
        ...
        if True:
            ...
            context_shape[0] = context_shape[0]*f
            context = context.expand(tuple(context_shape)) 

        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p13", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p14", color="blue")
        ...
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="p15", color="blue")

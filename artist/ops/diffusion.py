...
class GaussianDiffusion(object):
    ...
    def __init__(self,
        ...
        # alphas
        betas = betas.to(torch.device('cuda:0'))
        alphas = (1 - self.betas).to(torch.device('cuda:0'))
    ...
    def p_mean_variance(self, xt, t, model, model_kwargs={}, clamp=None, percentile=None, guide_scale=None, torch_stream=None):
        ...
        if self.var_type == 'learned':
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="3", color="blue")
            
            out, log_var = out.chunk(2, dim=1)
            var = torch.exp(log_var)
        elif self.var_type == 'learned_range':
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="4", color="blue")
            ...
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="5", color="blue")
            
            var = _i(torch.cat([self.posterior_variance[1:2], self.betas[1:]]), t, xt)
            log_var = torch.log(var)
        elif self.var_type == 'fixed_small':
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="6", color="blue")
        ...
        if self.mean_type == 'x_{t-1}':
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="7", color="blue")
            ...
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="8", color="blue")
            
            x0 = out
            mu, _, _ = self.q_posterior_mean_variance(x0, xt, t)
        elif self.mean_type == 'eps':
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="9", color="blue")
            x0 = _i(self.sqrt_recip_alphas_cumprod, t, xt) * xt - \
                 _i(self.sqrt_recipm1_alphas_cumprod, t, xt) * out
            mu, _, _ = self.q_posterior_mean_variance(x0, xt, t)
        
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="10", color="blue")
    ...
    @torch.no_grad()
    def ddim_sample(self, xt, t, model, model_kwargs={}, clamp=None, percentile=None, condition_fn=None, guide_scale=None, ddim_timesteps=20, eta=0.0, torch_stream=None):
        ...
        rng = nvtx.start_range(message="p_mean_variance", color="red")
        # predict distribution of p(x_{t-1} | x_t)
        _, _, _, x0 = self.p_mean_variance(xt, t, model, model_kwargs, clamp, percentile, guide_scale, torch_stream=torch_stream)
        nvtx.end_range(rng)
        rng = nvtx.start_range(message="post/p_mean_variance", color="red")
        if condition_fn is not None:
            # x0 -> eps
    ...
    @torch.no_grad()
    def ddim_sample_loop(self, noise, model, model_kwargs={}, clamp=None, percentile=None, condition_fn=None, guide_scale=None, ddim_timesteps=20, eta=0.0, torch_stream=None):
        # prepare input
        import nvtx
        ...
        nvtx.end_range(rng)
        for step in steps:
            rng = nvtx.start_range(message="full", color="blue")
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            nvtx.end_range(rng)
            rng = nvtx.start_range(message="ddim_sample", color="blue")
            xt, _ = self.ddim_sample(xt, t, model, model_kwargs, clamp, percentile, condition_fn, guide_scale, ddim_timesteps, eta, torch_stream=torch_stream)
            nvtx.end_range(rng)
        return xt
    ...
    def _scale_timesteps(self, t):
        if self.rescale_timesteps:
            tt = t.float() * 1000.0 / self.num_timesteps
            return tt
        return t
        #return t.float()
...
class GaussianDiffusion_style(object):
    ...
    def _scale_timesteps(self, t):
        if self.rescale_timesteps:
            import nvtx
            rng = nvtx.start_range(message="timestep", color='blue')
            tt = t.float() * 1000.0 / self.num_timesteps
            nvtx.end_range(rng)
            return tt
        return t

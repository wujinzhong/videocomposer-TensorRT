...
class MiDaS(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"hp: {hp}")
        print(f"wp: {wp}")
        x11 = self.fc1(x1).permute(0, 2, 1)
        print(f"x11: {x11.shape}")
        x11_shape = x11.shape 
        x1 = x11.reshape(x11_shape[0], x11_shape[1], hp, wp)
        #x1 = x11.unflatten(2, (hp, wp))
        x1 = self.conv1(x1)

        # stage2
        x = self.blocks[self.slices[1]](x)
        x2 = torch.cat([x[:, 1:], x[:, :1].expand_as(x[:, 1:])], dim=-1)
        x22 = self.fc2(x2).permute(0, 2, 1)
        x2 = x22.reshape((x22.shape[0], x22.shape[1], hp, wp))
        x2 = self.conv2(x2)

        # stage3
        x = self.blocks[self.slices[2]](x)
        x3 = torch.cat([x[:, 1:], x[:, :1].expand_as(x[:, 1:])], dim=-1)
        x33 = self.fc3(x3).permute(0, 2, 1)
        x3 = x33.reshape((x33.shape[0], x33.shape[1], hp, wp))
        x3 = self.conv3(x3)

        # stage4
        x = self.blocks[self.slices[3]](x)
        x4 = torch.cat([x[:, 1:], x[:, :1].expand_as(x[:, 1:])], dim=-1)
        x44 = self.fc4(x4).permute(0, 2, 1)
        x4 = x44.reshape((x44.shape[0], x44.shape[1], hp, wp))
        x4 = self.conv4(x4)
